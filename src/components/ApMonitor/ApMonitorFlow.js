...
//
export default class ApMonitorFlow extends React.Component {
	//
	constructor(props) {
		super(props);
		this.chart = null;
	...
	//
	initChart() {
		this.chart = echarts.init(document.getElementById('echarts_ApMonitorFlow'));
	}
	...
	setChart(time) {
		...
		this.chartOptions = {
				legend: {
					...
					orient: 'vertical',
					formatter: (name) => {
						return (name + '：' + ((name == '上行流量') ? '{value|500KB/s}' : '{value|400KB/s}'));
					},
					textStyle: {
						rich: {
							value: {
	...
	render() {
		return (
			<div>
				<div id="echarts_ApMonitorFlow" style={{height: 300}}>
				</div>
			</div>
		);
	}
...
//
ApMonitorFlow.propTypes = {
	curTime: PropTypes.number.isRequired,
};

ApMonitorFlow.defaultProps = {
	curTime: 1
};