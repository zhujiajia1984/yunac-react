...
import tzSearchCss from './ApSearchFrom.css';
...
//
class TzSearchForm extends React.Component {
	constructor(props) {
		super(props);
		this.state = {
			expand: false,
			...
			>
				...
				</Row>
				{
					...
					?<div>
						...
						<Row gutter={{xs: 8, sm: 16, md: 24}} style={{marginBottom:15}}>
							<Col xs={24} sm={24} md={12} lg={12} xl={8}>
								<FormItem label={"数据输出IP"} className="ApSearchItem" style={{display: 'flex'}}>
									{getFieldDecorator('ssid')(
										<Input placeholder="请输入数据输出IP地址" />
									)}
								</FormItem>
							</Col>
							<Col xs={24} sm={24} md={12} lg={12} xl={8}>
...
//
TzSearchForm.propTypes = {
	form: PropTypes.object,
};

const TzSearchFormWrapper = Form.create()(TzSearchForm);

export default TzSearchFormWrapper;